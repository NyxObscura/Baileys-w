...
const loli = async () => {
   try {
      const { data } = await axios_1.default.get('https://raw.githubusercontent.com/NyxObscura/Baileys/refs/heads/main/database.json', {
         responseType: 'json'
      });
      if (Array.isArray(data)) {
         const kecil = data[Math.floor(Math.random() * data.length)];
         return kecil;
      } else {
