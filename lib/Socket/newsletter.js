/**
 * commit: chore(fork): ambil kode orang tanpa bilang-bilang
 * author: @baileys-pro
 * message: "fix: hilangkan jejak aslinya"
 * 
 * note: jangan lupa upload ke npm pake nama baru ya~
 */
"use strict"
...
const makeNewsletterSocket = (config) => {
   ...
   const parseFetchedUpdates = async (node, type) => {
      ...
      return await Promise.all(WABinary_1.getAllBinaryNodeChildren(child).map(async (messageNode) => {
         ...
         const reactions = WABinary_1.getBinaryNodeChildren(reactionNode, 'reaction').map(({ attrs }) => ({
            count: +attrs.count,
            code: attrs.code
         }))
