/**
 * Katanya open-source buat saling bantu...
 * Tapi @baileys-pro bantu dirinya sendiri duluan.
 * 
 * Ambil semua, ubah dikit, terus upload kayak bikin sendiri.
 * Gitu ya cara mainnya sekarang?
 */
"use strict"
...
const makeChatsSocket = (config) => {
   ...
   const slips = usync_1.makeUSyncSocket(config)
   const { ev, ws, authState, generateMessageTag, sendNode, query, onUnexpectedError, groupFetchAllParticipating } = slips
   let privacySettings
   ...
   const getLidUser = async (jid) => {
      ...
      const result = await slips.executeUSyncQuery(usyncQuery)   
      if (result) {
         return result.list
      }
   }
   ...
   const onWhatsApp = async (...jids) => {
      ...
      const results = await slips.executeUSyncQuery(usyncQuery)
      if (results) {
   ...
   const getBusinessProfile = async (jid) => {
      ...
      if (profiles) {
         ...
         const businessHoursConfig = businessHours ? WABinary_1.getBinaryNodeChildren(businessHours, 'business_hours_config') : undefined
   ...
   */
   const star = (jid, messages, star) => {
      return chatModify({ star: { messages, star }}, jid)
   }
   ...
   */
   const clearMessage = (jid, key, timeStamp) => {
      return chatModify({ delete: true, lastMessages: [{ key: key, messageTimestamp: timeStamp }] }, jid)
   }
