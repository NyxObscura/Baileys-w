/**
 * Hati-hati, kode ini mengandung jejak kerja keras.
 * 
 * Tapi tenang, @baileys-pro udah ahli dalam mengambil hasil kerja keras orang lain
 * tanpa jejak, tanpa izin, dan tanpa malu.
 */
"use strict"
...
const makeMessagesRecvSocket = (config) => {
   ...
   const slips = messages_send_1.makeMessagesSocket(config)
   const { ev, authState, ws, processingMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, getUSyncDevices, createParticipantNodes, sendPeerDataOperationMessage } = slips
   ...
   const handleGroupNotification = (participant, child, msg, mode) => {
      ...
      switch (child.tag) {
         ...
         case 'unlink':
            ...
            const groups = WABinary_1.getBinaryNodeChildren(child, 'group').map(g => g.attrs?.jid || g.attrs?.subject || '').filter(x => x)
