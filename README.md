<div align="center">
  <img src="https://c.tenor.com/tiHhjlsWRmcAAAAC/tenor.gif" alt="animated banner" />
...
This Baileys has been re-modified by [@NyxObscura](https://github.com/NyxObscura). you are free to use, modify, copy, and publish it, but do not use it for illegal purposes, sell it, monetize it, or turn it into a closed-source project.
...
```
yarn add @NyxObscura/baileys-md
```

Use the edge version (no guarantee of stability, but latest fixes + features)
```
yarn add github:NyxObscura/baileys-md
```

Then import your code using:
```ts 
const makeWASocket = require('@NyxObscura/baileys-md')
```
...
```ts
const makeWASocket, { useMultiFileAuthState } = require('@NyxObscura/baileys-md')
...
```ts
const makeWASocket, { DisconnectReason, useMultiFileAuthState } = require('@NyxObscura/baileys-md')
...
```ts
const makeWASocket, { useSingleFileAuthState, useMongoFileAuthState } = require('@NyxObscura/baileys-md')
...
const connectAuth = async() => {
  ...
  const collection = client.db("@kasumi").collection("sessions")
  return collection
}
...
const { makeInMemoryStore, getAggregateVotesInPollMessage } = require('@NyxObscura/baileys-md');

const logger = pino({ timestamp: () => `,"time":"${new Date().toJSON()}"` }).child({ class: "@kasumi" });
logger.level = "fatal";
...
```ts
const makeWASocket, { makeInMemoryStore } = require('@NyxObscura/baileys-md')
...
### Buttons Interactive Message ( with other options )
```ts
...
await sock.sendStatusMentions(jid, {
   image: {
      url: 'https://api.ssateam.my.id.jpg'
   },
   caption: 'Hi'
}, 'Check this out')
```
...
await sock.sendAlbumMessage(jid, [
   {
      type: 'image', or video
      data: Buffer // or data: { url: 'https://example.jpg' }
      caption: 'caption'
   }
...
### Shop Message ( with other options )
```ts
// Image
await sock.sendMessage(jid, {
   image: {
...
const { downloadMediaMessage, getContentType } = require('@NyxObscura/baileys-md')

// Use the built-in logger or console if not available
const logger = console

sock.ev.on('messages.upsert', async ({ messages: [m] }) => {
   try {
      if (!m.message) return // stop if the message is empty
      const messageType = getContentType(m.message) // get the message type
      ...
      if (messageType === 'imageMessage') {
         ...
         const stream = await downloadMediaMessage(m, 'stream', {}, {
            logger,
            reuploadRequest: sock.updateMediaMessage // request reupload if media is deleted
         })
         ...
         writeStream.on('finish', () => {
            console.log('Image successfully downloaded and saved as my-download.jpeg')
         })
      }
   } catch (error) {
...
await sock.sendMessage(jid, { disappearingMessagesInChat: WA_DEFAULT_EPHEMERAL }) // this is 1 week in seconds -- how long you want messages to appear for
...
await sock.groupParticipantsUpdate(jid, ['abcd@s.whatsapp.net', 'efgh@s.whatsapp.net'], 'add' /** replace this parameter with 'remove' or 'demote' or 'promote'*/ )
```
### Change Subject (name)
```ts
...
### Change Add Mode
```ts
await sock.groupMemberAddMode(jid,
...
Thank you to ***Stigma Software Archive*** and also to the other Baileys developers

<table>
  <tr>
    <td align="center">
      <img src="https://raw.githubusercontent.com/NyxObscura/Database-doang-sih/main/uploads/1741115563722.jpeg" width="100" />
      <br />
      <b>NyxObscura</b>
    </td>
        <td align="center">
      <a href="https://github.com/NyxObscura">
        <img src="https://github.com/NyxObscura.png" width="100" />
      </a>
      <br />
      <b>NyxObscura</b>
    </td>
    <td align="center">
      <a href="https://github.com/Itsukichann">
        <img src="https://github.com/Itsukichann.png" width="100" />
      </a>
      <br />
      <b>Itsukichann</b>
    </td>
...
<p align="center" style="font-size: 12px;">
  © 2024 NyxObscura – All Rights Reserved
</p>